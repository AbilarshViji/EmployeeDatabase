    ...
    public void writeFile(String fileName) throws IOException {
        ...
        for (int i = 0; i < hashTable.buckets.length; i++) {
            for (int x = 0; x < hashTable.buckets[i].size(); x++) {
                if (hashTable.buckets[i].get(x).getHourlyWage() == null) {
                    String[] temp = new String[]{"F", hashTable.buckets[i].get(x).getEmployeeNum(), buckets[i].get(x).getFirstName(), buckets[i].get(x).getLastName(), buckets[i].get(x).getWorkLocation(), buckets[i].get(x).getDeductionRate(), buckets[i].get(x).getAnnualSalary()};
                    writer.writeNext(temp);
                } else {
                    String[] temp = new String[]{"P", hashTable.buckets[i].get(x).getEmployeeNum(), hashTable.buckets[i].get(x).getFirstName(), buckets[i].get(x).getLastName(), buckets[i].get(x).getWorkLocation(), buckets[i].get(x).getDeductionRate(), buckets[i].get(x).getHourlyWage(), buckets[i].get(x).getHoursPerWeek(), buckets[i].get(x).getWeeksPerYear()};
                    writer.writeNext(temp);
                }
